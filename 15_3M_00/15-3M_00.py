...
plt.show()
plt.draw()
fig1.savefig('Loss_Curve', dpi=100)
...
plt.close('all')
figtrain, ax1 = plt.subplots()
plt.scatter(Target_Test,Prediction_Test)
ax1.set_xlabel('SWL - Measured')
ax1.set_ylabel('SWL - Prediction')
plt.xlim(643.55, 643.90)
plt.ylim(643.55, 643.90)
figtrain.set_figheight(7)
figtrain.set_figwidth(10)
figtrain.tight_layout()
fig = plt.gcf()
plt.draw()
fig.savefig('scatter_test_set', dpi=100)
...
# LOAD THE TRAINED NETWORK PARAMETERS to a prediction model - FOR USER
#MLP_Prediction = Net(input_size, hidden_size01, hidden_size02, output_size)
#MLP_Prediction.load_state_dict(torch.load('Trained_G01_M01.pt'))

#Input_data = [P(t-8), P(t-7), P(t-6), P(t-5), P(t-4), P(t-3), P(t-2), P(t-1),P(t),
			   #ET(t-8), ET(t-7), ET(t-6), ET(t-5), ET(t-4), ET(t-3), ET(t-2), ET(t-1),ET(t),
			   #MWL(t-5), MWL(t-4), MWL(t-3), MWL(t-2), MWL(t-1)]
#Input_data = torch.FloatTensor(Input_data)
#Outputs_Modeled = MLP_Prediction(Input_data)
#print(Outputs_Modeled)
#print(len(Outputs_Modeled))
